...
- **Notify me when I'm up** — On a running tournament, anyone on the page can pick their name and get told when their court is called: the page chimes, buzzes, and shows a full-width callout, plus a real browser notification where the browser allows one. No sign-in — most players arrive by scanning the code and never make an account — and the choice is remembered across reloads. There's a "keep this screen on" toggle for a phone propped on a bench. It rides the realtime bracket updates the page already subscribes to, so it costs nothing to run; it can't reach a phone asleep in a pocket (browsers suspend background tabs, and iOS has no web notifications outside a home-screen app), which is what SMS would be for
- **Printable QR sign** — Every tournament gets a QR code pointing at its page. Organizers find it on the tournament's edit screen and can print a full-sheet sign (name, date, location, big code) to tape up at the check-in table, or download the bare code as PNG/SVG for a flyer or slide. Two versions print from the same screen — one that calls for sign-ups, one for tournament day ("scan to check your playing status and see the results") — either separately or both at once, since signs get made in one sitting before the event. The code is identical on both and carries the event from sign-ups through live scores
